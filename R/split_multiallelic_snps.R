...
#'
#' @description Changes the single line representation of multiallelic sites to a multi line
#' representation. That is, rows that have N alleles are replicated N-1 times.
#'
#' @param mat - matrix
...
split_multi_to_biallelic_snps = function(mat, ar_results){
  ...
  ar_results_split = ar_results[split_rows_flag, , drop=F]
  rownames(mat_split) = rownames(ar_results_split)

  return(list(mat_split = mat_split, ar_results_split = ar_results_split, split_rows_flag = split_rows_flag))
