...
#' Build tree
#'
#' Builds maximum likelihood tree using \code{\link[phangorn]{optim.pml}} with a GTR model of nucleotide substitution.
#'
#' @param mat allele matrix where rows are samples and columns are variants (generated by \code{load_vcf_file})
#'
#' @return maximum likelihood tree generated by \code{\link[phangorn]{optim.pml}}
#' @export
#'
#' @examples
#' ML R code inspired by: https://www.molecularecologist.com/2016/02/quick-and-dirty-tree-building-in-r/
build_tree = function(mat){
  ...
  invisible(capture.output(fitGTR = phangorn::optim.pml(fit,model='GTR',rearrangement = 'stochastic')))
  return(fitGTR$tree)
}
