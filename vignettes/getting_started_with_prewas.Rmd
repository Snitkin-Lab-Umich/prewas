...
Choosing a reference allele can be particularly important when doing gene-based analysis and therefore aggregating SNPs by gene. We suggest to reference to the ancestral allele. In cases where ancestral reconstruction is not feasible (e.g. computational intensity) or low confidence, we suggest referencing to the major allele instead of referencing to an arbitrary reference genome. Thus, when ancestral reconstruction is not used (anc = FALSE), prewas will use the major allele as the reference allele. 
...
Below, we'll explore some usage examples. 
