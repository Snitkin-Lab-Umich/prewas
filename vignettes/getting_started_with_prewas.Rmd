...
prewas creates a variant matrix (where each row is a variant, each column is a sample, and the entries are presence - 1 - or absence - 0 - of the variant) that can be used as input for bGWAS tools, such as hogwash or treeWAS.  
...
An visualization of these 3 steps in the prewas workflow is shown below where B shows how prewas multialleleic sites, C shows options for referencing to the ancestral alleles and D shows how prewas can group SNPs into genes. 
...
![detailed prewas workflow](Figure_S1.png){width=50%}
