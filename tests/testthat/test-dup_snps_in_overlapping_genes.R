...
test_that("dup_snps_in_overlapping_genes() gives error when given invalid inputs", {
  ...
  expect_error(dup_snps_in_overlapping_genes(matrix(0, 10, 10),
                                             matrix(0, 10, 10)))
})
...
test_that("dup_snps_in_overlapping_genes_snpeff() works as expected when given valid inputs", {
  ...
  temp_predicted_impact <-
    c("LOW", "LOW|HIGH", "MODERATE", "HIGH|HIGH", "MODIFIER", "LOW", "LOW|HIGH")
  temp_gene <- c("gene1",
                 "gene2|gene3",
                 "gene4",
                 "gene5|gene6",
  ...
  temp_dup_mat <-
    dup_snps_in_overlapping_genes_snpeff(temp_bin_mat,
                                         temp_predicted_impact,
                                         temp_gene)
...
test_that("dup_snps_in_overlapping_genes() gives error when given invalid inputs", {
  ...
  temp_dup_mat <- dup_snps_in_overlapping_genes_snpeff(temp_bin_mat,
                                                       temp_predicted_impact,
                                                       temp_gene)

  expect_error(dup_snps_in_overlapping_genes_snpeff("foo", "foo", "foo"))
  expect_error(dup_snps_in_overlapping_genes_snpeff(matrix(0, 10, 10),
                                                    "foo",
                                                    "foo"))
})
